...
class BERTProcessor:
    ...
    def analyze(self, videos_df, comments_df):
        ...
        video_embeddings = self.extract_bert_embeddings(
            videos_df["Cleaned_Description"].tolist()
        )
        comment_embeddings = self.extract_bert_embeddings(
            comments_df["Cleaned_Comment"].tolist()
        )

        # Extract top keywords from the embeddings and get the scores
        top_video_indices, video_keyword_scores = self.extract_keywords_from_embeddings(
            video_embeddings, self.top_n
        )
        top_comment_indices, comment_keyword_scores = (
            self.extract_keywords_from_embeddings(comment_embeddings, self.top_n)
        )

        # Visualize and save results
        video_keywords = [
            videos_df["Cleaned_Description"].iloc[i] for i in top_video_indices
        ]
        comment_keywords = [
            comments_df["Cleaned_Comment"].iloc[i] for i in top_comment_indices
        ]

        self.plot_keywords(
            video_keywords,
            video_keyword_scores,
            "Top Keywords in Video Descriptions (BERT)",
            "bert_keywords_descriptions.png",
        )
        self.plot_keywords(
            comment_keywords,
            comment_keyword_scores,
            "Top Keywords in Comments (BERT)",
            "bert_keywords_comments.png",
        )
