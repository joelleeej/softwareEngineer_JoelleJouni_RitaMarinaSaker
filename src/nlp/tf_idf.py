...
class TFIDFProcessor:
    ...
    def apply_tfidf(self, videos_df, comments_df):
        combined_text = list(videos_df["Cleaned_Description"]) + list(
    ...
    def visualize_tfidf_keywords(
        self, tfidf_df, num_words=10, output_filename="tfidf_keywords_descriptions.png"
    ):
        ...
        ).generate_from_frequencies(sum_tfidf_scores)
    ...
    def visualize_video_description_keywords(
        self,
        videos_df,
        feature_names,
        tfidf_matrix,
        output_filename="tfidf_keywords_comments.png",
    ):
        ...
        description_keywords = sorted(
            description_keywords, key=lambda x: x[1], reverse=True
        )
