...
class TopicModelingProcessor:
    ...
    def apply_model_predictions(self, videos_df, comments_df):
        ...
        X_tfidf_comments = self.tfidf_vectorizer.transform(comments_df['Cleaned_Comment'])
        comments_df['Comment_Dominant_Topic'] = self.loaded_model.predict(X_tfidf_comments)
    ...
    def generate_summary(self, videos_df, comments_df):
        ...
        summary = f"The dominant topic in these columns are:{video_summary}{comment_summary}"
        dominant_topics = {
    ...
    def process_and_visualize(self, videos_df, comments_df):
        ...
        video_title_img = self.visualize_topics(videos_df, 'Cleaned_Title_Dominant_Topic',
                                                'Dominant Topics in Video Titles', 'video_title_img')
        video_description_img = self.visualize_topics(videos_df, 'Cleaned_Description_Dominant_Topic',
                                                      'Dominant Topics in Video Descriptions', 'video_description_img')
        comment_img = self.visualize_topics(comments_df, 'Comment_Dominant_Topic',
                                            'Dominant Topics in Comments', 'comment_img')
