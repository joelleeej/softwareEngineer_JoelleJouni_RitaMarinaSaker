...
class TestTopicModelingProcessor(unittest.TestCase):
    def setUp(self):
        ...
        self.processor = TopicModelingProcessor(
            model_dir="channel_analysis_models/", static_dir="mock_static_dir/"
        )
        ...
        self.processor.loaded_model.predict = Mock(
            ...
            * len(x)  # Generate predictions matching the input length
        )
    ...
    def test_apply_model_predictions(self):
        ...
        videos_df, comments_df = self.processor.apply_model_predictions(
    ...
    def test_generate_summary(self):
        ...
        with patch.object(
            videos_df["Cleaned_Title_Dominant_Topic"],
            "mode",
            return_value=pd.Series([0]),
        ):
            summary, dominant_topics = self.processor.generate_summary(
                videos_df, comments_df
            )
