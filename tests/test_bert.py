...
class TestBERTProcessor(unittest.TestCase):
    ...
    def test_preprocess_data(self):
        videos_df, comments_df = self.bert_processor.preprocess_data(self.videos_df, self.comments_df)
        self.assertIn('Cleaned_Description', videos_df.columns)
        self.assertIn('Cleaned_Comment', comments_df.columns)
        self.assertEqual(videos_df['Cleaned_Description'].iloc[0], "this is the first video description")
    ...
    def test_extract_bert_embeddings(self, mock_bert_model, mock_bert_tokenizer):
        ...
        embeddings = self.bert_processor.extract_bert_embeddings(["This is a test sentence."])
    ...
    def test_plot_keywords(self):
        ...
        scores = [0.9, 0.8, 0.7]
        try:
            self.bert_processor.plot_keywords(keywords, scores, "Test Plot", "test_plot.png")
