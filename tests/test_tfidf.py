...
class TestTFIDFProcessor(unittest.TestCase):
    ...
    def test_clean_data(self):
        cleaned_videos_df, cleaned_comments_df = self.tfidf_processor.clean_data(
        ...
        self.assertNotEqual(
            self.videos_df["description"][0],
            cleaned_videos_df["Cleaned_Description"][0],
        )

    def test_apply_tfidf(self):
        self.tfidf_processor.clean_data(self.videos_df, self.comments_df)
        tfidf_df, feature_names, tfidf_matrix = self.tfidf_processor.apply_tfidf(
            self.videos_df, self.comments_df
        )
    ...
    def test_visualize_tfidf_keywords(self):
        ...
        output_path = os.path.join(
            self.tfidf_processor.static_dir, "tfidf_keywords_descriptions.png"
        )
    ...
    def test_visualize_video_description_keywords(self):
        ...
        self.tfidf_processor.visualize_video_description_keywords(
            self.videos_df, feature_names, tfidf_matrix
        )
        output_path = os.path.join(
            self.tfidf_processor.static_dir, "tfidf_keywords_comments.png"
        )
        self.assertTrue(os.path.exists(output_path))
